...
public class LoaiMonAnDatabase {
	...
	public ArrayList<LoaiMonAn> getAllLoaiMonAn() {
		try {
			...
			while (rs.next()) {
				LoaiMonAn s = new LoaiMonAn(rs.getString("MaLoai"),
						rs.getString("TenLoai"), rs.getString("Vung"),
						rs.getString("MoTa"));
	...
	public String getTenloai(String maloai) {
		try {
			ResultSet rs = lib
					.executeQuery("Select TenLoai from LoaiMonAn where Maloai like '%"
							+ maloai + "%'");
			String s = "";
			while (rs.next()) {
				s = rs.getString(1);
	...
	public LoaiMonAn getMaloai(int i) {
		try {
			ResultSet rs = lib.executeQuery("Select * from LoaiMonAn limit "
					+ i + "," + 1);
			LoaiMonAnCollection collection = new LoaiMonAnCollection();
			LoaiMonAn s = new LoaiMonAn();
			while (rs.next()) {
				s = new LoaiMonAn(rs.getString(1), rs.getString(2),
	...
	public int InsertLoaiMonAn(LoaiMonAn s) {
		try {
			int result = lib
					.executeUpdate("INSERT INTO LoaiMonAn(MaLoai, TenLoai, Vung, MoTa) VALUES("
							+ "'"
							+ s.getMaLoaiMonAn()
							+ "'"
							+ ",'"
	...
	public int DeleteLoaiMonAn(LoaiMonAn s) {
		try {
			int result = lib
					.executeUpdate("DELETE FROM LoaiMonAn WHERE MaLoai=" + "'"
							+ s.getMaLoaiMonAn() + "'");
			lib.close();
			return result;
		} catch (Exception e) {
	...
	public int UpdateLoaiMonAn(LoaiMonAn s) {
		try {
			int result = lib.executeUpdate("UPDATE LoaiMonAn SET "
					+ "TenLoai='" + s.getTenLoai() + "'" + ", Vung='"
					+ s.getVung() + "'" + ", MoTa='" + s.getMoTa() + "'"
