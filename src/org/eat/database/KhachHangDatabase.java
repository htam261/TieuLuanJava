...
public class KhachHangDatabase {
	...
	public ArrayList<KhachHang> getAllKhachHang() {
		try {
			ResultSet rs = lib
					.executeQuery("SELECT * FROM KhachHang WHERE LaNhanVien=false");
			KhachHangCollection collection = new KhachHangCollection();
			while (rs.next()) {
				KhachHang s = new KhachHang(rs.getString("ID"),
	...
	public int UpdateKhachHang(KhachHang s) {
		try {
			int result = lib.executeUpdate("UPDATE KhachHang SET " + "HoTen='"
					+ s.getHoTen() + "'" + ", DiaChi='" + s.getDiaChi() + "'"
					+ ", Email='" + s.getEmail() + "'" + ", SoDienThoai='"
					+ s.getSoDienThoai() + "'" + " WHERE ID='" + s.getId()
					+ "'");
			lib.close();
			return result;
		} catch (Exception e) {
