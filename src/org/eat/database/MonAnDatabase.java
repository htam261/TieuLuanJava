...
public class MonAnDatabase {
	...
	public ArrayList<MonAn> getAllMonAn() {
		try {
			...
			while (rs.next()) {
				MonAn s = new MonAn(rs.getString("MaMonAn"),
						rs.getString("TenMonAn"), rs.getString("HinhAnh"),
						rs.getString("Loai"), rs.getString("MoTa"),
						rs.getDouble("Gia"));
	...
	public ArrayList<MonAn> getMonAn(String mamon) {
		try {
			ResultSet rs = lib
					.executeQuery("SELECT * FROM MonAn where MaMonAn like '%"
							+ mamon + "%'");
			MonAnCollection collection = new MonAnCollection();
			while (rs.next()) {
				MonAn s = new MonAn(rs.getString("MaMonAn"),
	...
	public ArrayList<MonAn> getSearch(String maloai, int gia) {
		try {
			...
			if (gia == 0)
				...
				str = " gia > 0";
			ResultSet rs;
			sql = "SELECT * FROM MonAn where Loai like '%" + maloai + "%' and "
					+ str;
	...
	public ArrayList<MonAn> getSearch(String maloai, String tenmonan, int gia) {
		try {
			...
			ResultSet rs;
			sql = "SELECT * FROM MonAn where Loai like '%" + maloai + "%' and "
					+ str + " and tenmonan like '%" + tenmonan + "%'";
			rs = lib.executeQuery(sql);
			ArrayList<MonAn> list = new ArrayList<MonAn>();
			while (rs.next()) {
				MonAn s = new MonAn(rs.getString("MaMonAn"),
	...
	public int InsertMonAn(MonAn s) {
		try {
			int result = lib
					.executeUpdate("INSERT INTO MonAn(MaMonAn, TenMonAn, HinhAnh, Loai, MoTa, Gia) VALUES("
							+ "'"
							+ s.getMaMonAn()
							+ "'"
							+ ",'"
	...
	public int UpdateMonAn(MonAn s) {
		try {
			int result = lib.executeUpdate("UPDATE MonAn SET " + "TenMonAn='"
					+ s.getTenMonAn() + "'" + ", HinhAnh='" + s.getHinhAnh()
					+ "'" + ", Loai='" + s.getLoai() + "'" + ", MoTa='"
					+ s.getMoTa() + "'" + ", Gia=" + s.getGia() + ""
					+ " WHERE MaMonAn='" + s.getMaMonAn() + "'");
			System.out.println("UPDATE MonAn SET " + "TenMonAn='"
					+ s.getTenMonAn() + "'" + ", HinhAnh='" + s.getHinhAnh()
					+ "'" + ", Loai='" + s.getLoai() + "'" + ", MoTa='"
					+ s.getMoTa() + "'" + ", Gia=" + s.getGia() + ""
					+ " WHERE MaMonAn='" + s.getMaMonAn() + "'");
