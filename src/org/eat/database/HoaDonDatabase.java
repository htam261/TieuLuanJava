...
public class HoaDonDatabase {
	...
	public ArrayList<HoaDon> getAllHoaDon() {
		try {
			...
			while (rs.next()) {
				HoaDon s = new HoaDon(rs.getString("MaHoaDon"),
						rs.getString("TenHoaDon"), rs.getString("KhachHang"),
						rs.getDate("NgayTao"), rs.getBoolean("DaThanhToan"),
	...
	public int InsertHoaDon(HoaDon s) {
		try {
			int result = lib
					.executeUpdate("INSERT INTO HoaDon(MaHoaDon, TenHoaDon, KhachHang, NgayTao, DaThanhToan, TongGia) VALUES("
							+ "'"
							+ s.getMaHoaDon()
							+ "'"
							+ ",'"
							...
							+ "'"
							+ ",'"
							+ s.getNgayTao().getYear()
							+ "/"
							+ s.getNgayTao().getMonth()
							+ "/"
							+ s.getNgayTao().getDate()
							+ "'"
							+ ","
							+ s.isDaThanhToan()
							+ ""
	...
	public int UpdateHoaDon(HoaDon s) {
		try {
			int result = lib.executeUpdate("UPDATE HoaDon SET "
					...
					+ ", DaThanhToan=" + s.isDaThanhToan() + "" + ", TongGia="
					+ s.getTongGia() + "" + " WHERE MaHoaDon='"
					+ s.getMaHoaDon() + "'");
			lib.close();
			return result;
		} catch (Exception e) {
