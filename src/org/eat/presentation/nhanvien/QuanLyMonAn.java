...
public class QuanLyMonAn extends JFrame implements ActionListener,
		ListSelectionListener {
	public QuanLyMonAn(String title) {
	...
	public void addMenu() {/*
							 * menubar = new JMenuBar(); mnuFile = new
							 * JMenu("File"); mnuFileWriteoDisk = new
							 * JMenuItem("Write Data to disk");
							 * mnuFileOpenDateFromDisk = new
							 * JMenuItem("Open Data from disk"); mnuFileExit =
							 * new JMenuItem("Exit"); menubar.add(mnuFile);
							 * mnuFile.add(mnuFileWriteoDisk);
							 * mnuFile.add(mnuFileOpenDateFromDisk);
							 * mnuFile.addSeparator(); mnuFile.add(mnuFileExit);
							 * setJMenuBar(menubar);
							 */
	}
	...
	public void loadTable() {
		...
		for (int i = 0; i < colma.size(); i++) {
			...
			dtmHoaDon.addRow(new Object[] { m.getMaMonAn(), m.getTenMonAn(),
					m.getHinhAnh(), l, m.getMoTa(), m.getGia() });
		}
	}
	...
	private String[] header = new String[] { "Mã món ăn", "Tên món ăn",
			"Hình ảnh", "Loại", "Mô tả", "Giá" };
