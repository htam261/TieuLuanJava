...
public class mnHoaDon extends JInternalFrame implements ActionListener,
	...
	public void addControl(String title) {
			...
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					if (lstHoaDon.getSelectedIndex() != -1) {
						...
						sorter.setRowFilter(RowFilter.regexFilter(h
								.getMaHoaDon()));
					}
				}
			}
		...
		tblDatMonAn.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				/*
				 * if (tblDatMonAn.getSelectedRow() != -1) { int row =
				 * tblDatMonAn.getSelectedRow(); MonAn m = new
				 * MonAn(tblDatMonAn.getValueAt(row, 0).toString() ,
	...
	public void loadTable() {
		...
		if (hdSelected == null) {
			for (int i = 0; i < coldma.size(); i++) {
				...
				dtmDatMonAn.addRow(new Object[] { d.getMaHoaDon(),
						m.getMaMonAn(), m.getTenMonAn(), d.getSoLuong(),
						l.getTenLoai(), m.getGia(), m.getMoTa() });
			}
		} else {
			DatMonAnCollection cold = new DatMonAnCollection(
					coldma.getAllDatMonAn_MaHoaDon(hdSelected.getMaHoaDon()));
		...
		if (e.getSource().equals(btnCateNew)) {
			...
			if (maSelected != null) {
				int option = JOptionPane.showConfirmDialog(null,
						"Bạn có chắc xóa món ăn: " + maSelected.getMaMonAn(),
	...
	private String[] header = new String[] { "Mã hóa đơn", "Mã món ăn",
			"Tên món ăn", "Số lượng", "Loại", "Giá", "Mô tả" };
	...
	private JButton btnCateRemove, btnCateNew, btnCateUpdate, btnNew, btnSave,
			btnRemove;
