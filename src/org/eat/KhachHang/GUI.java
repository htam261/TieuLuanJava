...
public class GUI extends JFrame implements ActionListener {
	...
	private String makh, maHD = "", maDMA = "", maMA = "", tenMA = "",
			maMAdb = "";
	...
	public void drawGUI() {
		...
		tblmonan.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				try {
					DefaultTableModel model = (DefaultTableModel) tblmonan
							.getModel();
					maMA = model.getValueAt(tblmonan.getSelectedRow(), 0)
							.toString();
					tenMA = model.getValueAt(tblmonan.getSelectedRow(), 1)
							.toString();
					giaMA = Double.parseDouble(model.getValueAt(
							tblmonan.getSelectedRow(), 4).toString());
	...
	public void actionPerformed(ActionEvent e) {
			...
			if (txtsoluong.getText().equalsIgnoreCase("")) {
				...
				if (maHD.equalsIgnoreCase("")) {
					...
					if (da.InsertDatMonAn(dma) > 0) {
						...
						JOptionPane.showMessageDialog(rootPane,
								"Đặt thành công");
						ChiTietDatMon ctdm = new ChiTietDatMon(maMA, tenMA,
								dma.getSoLuong(), giaMA);
					...
					DatMonAn dma = new DatMonAn(maDMA, maHD, maMA,
							Integer.parseInt(txtsoluong.getText()), d);
					boolean f = true;
		...
		if (e.getSource() == btnthanhtoan) {
			...
			JOptionPane.showMessageDialog(rootPane,
					"Số tiền quý khách cần thanh toán là " + tong + "VND");
